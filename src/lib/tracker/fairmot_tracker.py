...
class Tracker(object):
    def __init__(self, opt, device, frame_rate=30):
        ...
        self.frame_id = 0
        self.det_thresh = opt.conf_thres # confidence thresh for tracking
        self.buffer_size = int(frame_rate / 30.0 * opt.track_buffer)  # tracking buffer
        self.max_time_lost = self.buffer_size
        self.max_per_image = opt.K  # max number of output objects
    ...
    def pre_process(self, im0):
        ...
        # Padded resize
        im = letterbox(im0, self.opt.img_size)[0]
    ...
    def process_frame(self, im0):
        ...
        # Inference
        online_targets = self.update(im, im0)
        online_tlwhs = []
        online_ids = []
        #online_scores = []
        ...
        # save results
        results = (online_tlwhs, online_ids)
        #results.append((online_tlwhs, online_ids, online_scores))
        return results
