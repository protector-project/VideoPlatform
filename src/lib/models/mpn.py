...
class convAE(torch.nn.Module):
    def __init__(
        ...
        self.prototype = Meta_Prototype(
            proto_size, feature_dim, key_dim, temp_update, temp_gather
        )
        # output_head
...
def train_init(
    model, model_weights, model_alpha, loss_fn, img, lh_img, gt, lh_gt, idx, args
):
    ...
    loss = (
        args.loss_fea_reconstruct * fea_loss
        + args.loss_distinguish * dis_loss
        + args.loss_fra_reconstruct * loss_pixel
    )
    ...
    update_weights = OrderedDict(
        ...
        for ((name, param), (_, meta_alpha), grad) in zip(
            model_weights.items(), model_alpha.items(), grads
        )
    )
    ...
    lh_loss = (
        args.loss_fea_reconstruct * lh_fea_loss
        + args.loss_distinguish * lh_dis_loss
        + args.loss_fra_reconstruct * lh_loss_pixel
    )
...
def test_init(model, model_weights, model_alpha, loss_fn, imgs, gts, args):
    ...
    for j in range(args.test_iter):
        ...
        update_weights = OrderedDict(
            (name, param - torch.mul(meta_alpha, grad))
            for ((name, param), (_, meta_alpha), grad) in zip(
                model_weights.items(), model_alpha.items(), k_grads
            )
        )
...
def get_mpn(c=3, t_length=5, psize=10, fdim=[128], pdim=[128]):
    # Create model
