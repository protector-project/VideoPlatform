...
class TemporalTransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, dropout=None):
        ...
        print('dropout used in TemporalTransformer:{}'.format(dropout))
        self.width = width
        self.layers = layers
...
class visual_prompt(nn.Module):
    def __init__(self, sim_head, clip_state_dict, T, dropout=0.):
        super().__init__()
        ...
        if self.sim_header == "LSTM" or self.sim_header == "Transf" or self.sim_header == "Transf_cls" or self.sim_header == "Conv_1D" :
            ...
            # added for dropout
            if dropout > 0.:
                dpr = [x.item() for x in torch.linspace(0, dropout, transformer_layers)]  # stochastic depth decay rule
            else:
                dpr = None
            #
            
        if self.sim_header == "Transf" :
            self.transformer = TemporalTransformer(width=embed_dim, layers=6, heads=transformer_heads, dropout=dpr)
            print('layer=6')
