...
def main(opt):
	...
	traj_anomaly_detector = TrajAnomalyDetector(opt, device)
	...
	for i, (path, im0s, vid_cap, s) in enumerate(dataset):
		if frame_id in scene_df["frame"].values:
			...
			for track_id in track_ids:
				...
				if len(obs_df) >= opt.obs_len and len(pred_df) >= opt.pred_len:
					obs_traj = (
						obs_df[["x", "y"]]
						.to_numpy()
						.astype("float32")[-opt.obs_len :]
						.reshape(opt.obs_len, 2)
					)
					pred_traj = (
						pred_df[["x", "y"]]
						.to_numpy()
						.astype("float32")[: opt.pred_len]
						.reshape(opt.pred_len, 2)
					)
			...
			if trajectories:
				(
					...
					im0s, torch.Tensor(trajectories)
				)
